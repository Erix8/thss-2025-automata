...
class RepItem:
    """
    第三次实验新增，表示replacement中的一个元素。分为两种情况：
    - groupId为-1，此时content即为要替换的内容。
    ...
    @staticmethod
    def from_text(text: str) -> List["RepItem"]:
        """
        ...
        while text:
            ...
            if p == -1:
                ...
                break
            content_buf += text[:p]
            if p + 1 < len(text) and text[p + 1].isdigit():  # 形如$123
                q = p + 2
                while q < len(text) and text[q].isdigit(): q += 1
                group_id = int(text[p + 1:q])
                if content_buf:
                    r = RepItem()
                    r.content = content_buf
                    result.append(r)
                    content_buf = ""
                r = RepItem()
                r.groupId = group_id
                ...
                content_buf += "$"
                q = p + 2 if p + 1 < len(text) and text[p + 1] == "$" else p + 1  # 前者是$$的情况，后者是$a的情况
            text = text[q:]
...
class Regex:
    ...
    def replaceAll(self, text: str, replacement: List[RepItem]) -> str:
        """
        第三次实验新增
        ...
        :param text 输入的文本
        :param replacement 其中所有RepItem元素依次连接，即得到要把匹配内容最终替换为的结果。（RepItem的含义详见该类上的注释。）
        :return 替换后的文本
        """
        # TODO 请你完成这个函数
        return text
...
if __name__ == '__main__':
    ...
    elif typ == "matchAll": # 第三次实验新增
        result = regex.matchAll(input_str)
        print(json.dumps(result))
    elif typ == "replaceAll": # 第三次实验新增
        if replacement is None:
            raise ValueError("type=replaceAll的输入，但是未找到replacement字段！")
        result = regex.replaceAll(input_str, RepItem.from_text(replacement))
